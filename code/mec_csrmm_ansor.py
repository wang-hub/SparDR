...
def mec_csrmm(data,weight_data,weight_row,weight_col,weight_indptr,out_size):
    ...
    oshape = (1,out_channel,out_size, out_size)
...
def mec_csrmm_k1(data,weight_data,weight_indices,weight_indptr,out_size):
    ...
    return te.compute(oshape, f, tag="mec_csrmm_k1")
...
def test_sp_csrmm(data_shape,weight_data_shape,weight_row_shape,weight_col_shape,weight_indptr_shape,out_size):
    ...
    weight_indptr = te.placeholder(weight_indptr_shape,dtype='int32',name='w_indptr')
    conv = mec_csrmm(input_mec,weight_data,weight_row,weight_col,weight_indptr,out_size)
...
def conv(input_file=None,
    ...
    if (kISone):
        ...
        def apply_func(search_policy, state, stage_id):
            ...
            assert output.tag == "mec_csrmm_k1"
            ...
            s0.reorder(
                        output,
                        # [n,j_0, h_0, w_0, j_1, h_1, w_1, elem_idx, h_2, w_2],
                        [n ,j_0, elem_idx, h_0, w_0, j_1, h_1, w_1, h_2, w_2],
                    )
        ...
        task = tvm.auto_scheduler.SearchTask(
            func=test_sp_csrmm,
            args=(x.shape,csr.data.shape,weight_row.shape,weight_col.shape,csr.indptr.shape,out_size),
            target=target,
            task_inputs={
                "w_data": runtime.ndarray.array(csr.data),
