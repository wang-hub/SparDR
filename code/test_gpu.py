...
import mec_csrmm_gpu
import dense_gpu
from cupy_conv import test_cupy
from torch_conv import test_torch
...
Dir = "/root/user-data/TVM_GPU/code/modelCode/model/"
def testModel(modelName):
    # modelDir = Dir + (str)(modelName) + "-prune/"
    modelDir = Dir + (str)(modelName) 
    # modelPath = modelDir + "model-infer.onnx"
    modelPath = modelDir + "/model.onnx"

    # modelPath = '/root/user-data/ww/code/modelCode/model/vgg19/model.onnx'
    ...
    for res in op_res:
        ...
        if sparity > 0.8:
            ...
            if(False):
                ...
                ourTime = mec_csrmm_gpu.conv(input_file,kernel_file,P,S,kISone=False)
                torchTime = test_torch(input_file,kernel_file,P,S)
                cupyTime = test_cupy(input_file,kernel_file,P,S)
                denseConvTime = dense_gpu.test(input_file,kernel_file,P,S)
            #保存结果
            res = [ourTime,denseConvTime,torchTime,cupyTime]
            time_file = modelDir + "/time.txt"
            with open(time_file,'a') as f:
                f.writelines(kernelPath + ' ' + (str)(res))
                f.writelines("\n")
            print(res)
            timeRes.append(res)
    # #调优结果保存
